...
for n=3:nmax
    ...
    for i = 1:length(th2_values_rad)
        ...
        % Define options for fsolve to increase accuracy
        options = optimoptions('fsolve', 'TolFun', 1e-10, 'TolX', 1e-10, 'MaxIterations', 1000, 'Display', 'off');

        % Solve the equation with improved precision
        th1_values_rad(i) = fsolve(@(th1) equation(th1, th2), initial_guess, options);
    end
    
    % 将th1从弧度转换为角度
    th1_values_deg = 360-rad2deg(th1_values_rad);
    ...
    % 将数据首尾相接
    thn_p{1} = [th1_values_deg, flip(th1_values_deg), th2_values_deg, th3_values_deg, th3_values_deg, 360-flip(th2_values_deg)];
    thn_p{1} = [thn_p{1},360-thn_p{1}];
    for i = 2:n
    % 循环移位
